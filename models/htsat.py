...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., csa=False):
    ...
    def forward(self, x, mask=None):
        ...
        """
        B_, N, C = x.shape  # B_: batch_size * num_windows, N: Wh*Ww, C: dim
        qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)  # (3, B_, num_heads, N, head_dim)
        q, k, v = qkv[0], qkv[1], qkv[2]  # (B_, num_heads, N, head_dim)

        q = q * self.scale
        if self.csa:
            # print("We are using csa mechanism")
            k = k * self.scale
            q_attn = torch.matmul(q, q.transpose(-2, -1))  # (B_, num_heads, N, N)
            k_attn = torch.matmul(k, k.transpose(-2, -1))  # (B_, num_heads, N, N)
            attn = q_attn + k_attn   # (B_, num_heads, N, N)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, norm_before_mlp='ln', csa=False):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, csa=csa)
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 csa=(csa and i == depth-1))  # we only add csa in the last layer
...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def __init__(self, spec_size=256, patch_size=4, patch_stride=(4, 4),
                 ...
                 use_checkpoint=False, norm_before_mlp='ln', config=None, csa=True, **kwargs):   ## Change here to enable csa
        # !TODO change ways for enabling csa
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(self.embed_dim * 2 ** i_layer),
                               ...
                               csa=(i_layer==self.num_layers-1) if self.csa else False)

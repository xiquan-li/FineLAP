...
## calculate psds from scores and gt_dict obtained
def calculate_psds_from_score(score_buffer, gt_dict, audio_duration_file, fname_to_aid, save_dir): 
    psds1 = compute_psds_sed_scores(
                    scores=score_buffer,
                    ground_truth=gt_dict,
                    duration=audio_duration_file,
                    fname_to_aid=fname_to_aid,
                    dtc_threshold=0.7,
                    gtc_threshold=0.7,
                    save_dir=save_dir,
                    max_efpr=800)
    psds2 = compute_psds_sed_scores(
                    scores=score_buffer,
                    ground_truth=gt_dict,
                    duration=audio_duration_file,
                    fname_to_aid=fname_to_aid,
                    ...
                    save_dir=save_dir,
                    max_efpr=800)
    psds2021 = compute_psds_sed_scores(
                    scores=score_buffer,
                    ground_truth=gt_dict,
                    duration=audio_duration_file,
                    fname_to_aid=fname_to_aid,
    ...
    return psds1, psds2, psds2021

## generate ground truth data frame of AudioGroundingDataLoader
def generate_ground_truth_df(eval_dataloader):
    ground_truth = []
    for audio_item in eval_dataloader.dataset.data:
        ...
        for phrase_item in audio_item["phrases"]:
            ...
            for t_start, t_end in phrase_item["segments"]:
...
class Grounding_PrecisionRecall(PSDSEval):
    ...
    def _add_op(self, recall, precision, info=None):
        ...
        if set(op.keys()).isdisjoint(set(info.keys())):
            ...
            self.operating_points = \
                self.operating_points._append(op, ignore_index=True)
                # pd.concat([self.operating_points, pd.DataFrame([op])], ignore_index=True)
    ...
    def add_operating_point(self, detections, info=None):
        ...
        if not op_id:
            ...
            self.operating_points = self.operating_points._append(
                last_row, ignore_index=True)
            # self.operating_points = pd.concat([self.operating_points, pd.DataFrame(last_row)], ignore_index=True)
            return

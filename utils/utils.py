...
def all_gather_tensor(tensor):
    """
    Gather tensors from all ranks into a single list.
    """
...
def generate_length_mask(lens, max_length=None):
    ...
    if max_length is None:
        max_length = max(lens)
    # Create idxs on the same device as lens
    idxs = torch.arange(max_length, device=lens.device).repeat(N).view(N, max_length)  # (btz, max_length)
...
def set_logger(exp_name):
    log_output_dir = Path('exps', exp_name, 'logging')
    model_output_dir = Path('exps', exp_name, 'models')
...
def init_distributed_mode(args):
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        ...
        main_logger.info("Not using distributed mode")
        args["distributed"] = False
        return

    args["distributed"] = True
    
    # Set default dist_url if not provided
...
def t2a(audio_embs, cap_embs, return_ranks=False):
    ...
    audios = np.array([audio_embs[i] for i in range(0, audio_embs.shape[0], 5)])  # unique audio embeds

    ranks = np.zeros(5 * num_audios)  # the rank of correct caption w.r.t the audio
    top1 = np.zeros(5 * num_audios)

    for index in range(num_audios):

        # get query captions
        queries = cap_embs[5 * index: 5 * index + 5]  # [5, d]

        # compute scores
        # d = queries @ audios.T
        d = util.cos_sim(torch.Tensor(queries), torch.Tensor(audios)).numpy()  # [5, num_audios]

        inds = np.zeros(d.shape)  # [5, num_audios]
        for i in range(len(inds)):
            inds[i] = np.argsort(d[i])[::-1]  # ::-1 reverse list -> index in descending order
